#8/2/18: Giants vs Diamondbacks : Tony Randazzo

#164 pitches were called strikes/balls

#The robot-ump called 118 of those pitches as called strikes & 46 as balls

#Tony Randazzo called 115 of those pitches as called strikes & 49 as balls 

#Accuracy: 94%

Tony_Randazzo <- read.csv("~/Desktop/Analyzing Baseball Data with R/2018 Giants Umpires/Tony Randazzo_8:2.18.csv")
...
prediction_BG<-predict(tree_model, newdata = Tony_Randazzo_test, type = 'class')

confusionMatrix(table(prediction_BG, Tony_Randazzo_test$type))
confusionMatrix(prediction_BG,Tony_Randazzo_test$type)
...
plot(tree_model)
text(tree_model, use.n = T)
prediction_BG<-predict(tree_model, newdata = Tony_Randazzo_AdjustedCalls, type = 'class')
confusionMatrix(table(prediction_BG,Tony_Randazzo_AdjustedCalls$AdjustedCall))
