Mike_Estabrook <- read.csv("~/Desktop/Analyzing Baseball Data with R/2018 Giants Umpires/Mike_Estabrook_8:31:18.csv")
...
prediction_ME<-predict(tree_model, newdata = Mike_Estabrook_test, type = 'class')

confusionMatrix(table(prediction_ME, Mike_Estabrook_test$type))
confusionMatrix(prediction_ME,Mike_Estabrook_test$type)
...
plot(tree_model)
text(tree_model, use.n = T)
prediction_ME<-predict(tree_model, newdata = Mike_Estabrook_AdjustedCalls, type = 'class')
confusionMatrix(table(prediction_ME,Mike_Estabrook_AdjustedCalls$AdjustedCall))

# Mike Estaborook Strike Zone: 92.86% Accurate, 97.5% Precision, 91.76% Recall
