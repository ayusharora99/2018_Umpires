#8/27/18: Giants vs Mets : Lance Barrett

#126 pitches were called strikes/balls

#The robot-ump called 34 of those pitches as called strikes & 92 as balls

#Lance Barrett called 40 of those pitches as called strikes & 86 as balls 

#Accuracy: 90%

Lance_Barrett <- read.csv("~/Desktop/Analyzing Baseball Data with R/2018 Giants Umpires/Lance_Barrett_8:22:18.csv")

install.packages(c("e1071","caret","rpart"))

library(e1071)
library(caret)
library(rpart)

dim(Lance_Barrett)
names(Lance_Barrett)
is.na(Lance_Barrett)
colSums(is.na(Lance_Barrett)) 
Lance_Barrett = Lance_Barrett[,colSums(is.na(Lance_Barrett)) == 0] 
dim(Lance_Barrett)

# Subsetting Relevant Info
keeps = c("at_bat_number","player_name","pitch_type","plate_x","plate_z","zone","sz_top","sz_bot","type")
drops = c("event","des","hit_location","bb_type","on_3b","on_2b","on_1b","hc_x","hc_y","hit_distance_sc","launch_speed","launch_angle","estimated_ba_using_speedangle","estimated_woba_using_speedangle","woba_value","woba_denom","launch_speed_angle","iso_value","babip_value")
Lance_Barrett = Lance_Barrett[ , !(names(Lance_Barrett) %in% drops)]
# Lance_Barrett = Lance_Barrett[keep]
dim(Lance_Barrett)
Lance_Barrett

# Split Testing(28) & Training(108) data
# USE n fold cross validation or bootstrap if needed
Lance_Barrett_train = Lance_Barrett[0:(0.8 * nrow(Lance_Barrett)),]
dim(Lance_Barrett_train)
prop.table(table(Lance_Barrett_train$type))
Lance_Barrett_test = Lance_Barrett[(0.8*nrow(Lance_Barrett)):nrow(Lance_Barrett),]
dim(Lance_Barrett_test)
prop.table(table(Lance_Barrett_test$type))
# Decision Tree to Predict Umpire's Call 
tree_model <-rpart(type~., data = Lance_Barrett_train)
plot(tree_model)
text(tree_model, use.n = T)
prediciton_LB<-predict(tree_model, newdata = Lance_Barrett_test, type = 'class')

confusionMatrix(table(prediciton_LB, Lance_Barrett_test$type))
confusionMatrix(prediciton_LB,Lance_Barrett_test$type)
# 78.57% Accuracy, 82.35% Precision, 82.35% Recall w/ Keeps
# 100% in ALL w/ Drops
#sensitivity, specificity which is more important: 1.0000, 1.0000


#  Accuracy, precision, recall,sensitivity, specificity, ROC, AUC, Kappa, F score, LogLoss etc
# Run ROC for imbalanced data

# Kmeans Clustering
# https://www.youtube.com/watch?v=4R8nWDh-wA0&index=5&list=PLaozD487rOR_LgH8ObhA2-ns2ijiB-uaf 
# Hierarichal Clustering?

# Check for overfitting & underfitting

# Copy Pitch Calls into another data set and adjust type to the electronic strike zone calls

# Seperate Ball & Strike Types
Lance_Barrett_Strikes = subset(Lance_Barrett, Lance_Barrett$type == "S")
Lance_Barrett_Balls = subset(Lance_Barrett, Lance_Barrett$type == "B")

# Adjust call for Strikes 
# (plate_x < 0.833 & $plate_x > -0.833) & ($plate_z > sz_bot & plate_z < sz_top) == S
Lance_Barrett_Strikes$AdjustedCall = ifelse((Lance_Barrett_Strikes$plate_x < 0.833 & Lance_Barrett_Strikes$plate_x > -0.833) & (Lance_Barrett_Strikes$plate_z > Lance_Barrett_Strikes$sz_bot & Lance_Barrett_Strikes$plate_z < Lance_Barrett_Strikes$sz_top), 'S', 'B')
table(Lance_Barrett_Strikes$AdjustedCall)
# Adjust call for Balls
# (plate_x > 0.833 | $plate_x < -0.833) | ($plate_z < sz_bot | plate_z > sz_top) == B
Lance_Barrett_Balls$AdjustedCall = ifelse((Lance_Barrett_Balls$plate_x > 0.833 | Lance_Barrett_Balls$plate_x < -0.833)|(Lance_Barrett_Balls$plate_z < Lance_Barrett_Balls$sz_bot | Lance_Barrett_Balls$plate_z > Lance_Barrett_Balls$sz_top),'B','S')
table(Lance_Barrett_Balls$AdjustedCall)
# Merge to create new dataset
Lance_Barrett_AdjustedCalls = rbind(Lance_Barrett_Strikes,Lance_Barrett_Balls)

# Predict using Hunter Wendelstedt's Decision Tree on the modified dataset & map with adjusted_call to find margin of error
tree_model <-rpart(type~., data = Lance_Barrett)
plot(tree_model)
text(tree_model, use.n = T)
prediciton_LB<-predict(tree_model, newdata = Lance_Barrett_AdjustedCalls, type = 'class')
confusionMatrix(table(prediciton_LB,Lance_Barrett_AdjustedCalls$AdjustedCall))



