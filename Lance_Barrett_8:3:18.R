#8/3/18: Giants vs Diamondbacks : Lance Barrett

#137 pitches were called strikes/balls

#The robot-ump called 96 of those pitches as called strikes & 41 as balls

#Lance Barrett called 84 of those pitches as called strikes & 53 as balls 

#Accuracy: 90%

Lance_Barrett <- read.csv("~/Desktop/Analyzing Baseball Data with R/2018 Giants Umpires/Lance_Barrett_8:3:18.csv")
...
# Lance_Barrett = Lance_Barrett[keep]
dim(Lance_Barrett)
Lance_Barrett
...
prediction_BG<-predict(tree_model, newdata = Lance_Barrett_test, type = 'class')

confusionMatrix(table(prediction_BG, Lance_Barrett_test$type))
confusionMatrix(prediction_BG,Lance_Barrett_test$type)
...
plot(tree_model)
text(tree_model, use.n = T)
prediction_BG<-predict(tree_model, newdata = Lance_Barrett_AdjustedCalls, type = 'class')
confusionMatrix(table(prediction_BG,Lance_Barrett_AdjustedCalls$AdjustedCall))
