#7/28/18: Giants vs Brewers : Ryan Blakney

#153 pitches were called strikes/balls

#The robot-ump called 48 of those pitches as called strikes & 105 as balls

#Ryan Blakney called 59 of those pitches as called strikes & 94 as balls 

#Accuracy: 90%

Ryan_Blakney <- read.csv("~/Desktop/Analyzing Baseball Data with R/2018 Giants Umpires/Ryan_Blakney_7:28:18.csv")
...
prediction_BG<-predict(tree_model, newdata = Ryan_Blakney_test, type = 'class')

confusionMatrix(table(prediction_BG, Ryan_Blakney_test$type))
confusionMatrix(prediction_BG,Ryan_Blakney_test$type)
...
plot(tree_model)
text(tree_model, use.n = T)
prediction_BG<-predict(tree_model, newdata = Ryan_Blakney_AdjustedCalls, type = 'class')
confusionMatrix(table(prediction_BG,Ryan_Blakney_AdjustedCalls$AdjustedCall))
