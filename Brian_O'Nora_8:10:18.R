#8/10/18: Giants vs Pirates : Brian ONora

#186 pitches were called strikes/balls

#The robot-ump called 46 of those pitches as called strikes & 140 as balls

#Brian ONora called 53 of those pitches as called strikes & 133 as balls 

#Accuracy: 92%


Brian_ONora <- read.csv("~/Desktop/Analyzing Baseball Data with R/2018 Giants Umpires/Brian_O'Nora_8:10:18.csv")
...
prediciton_BN<-predict(tree_model, newdata = Brian_ONora_test, type = 'class')

confusionMatrix(table(prediciton_BN, Brian_ONora_test$type))
confusionMatrix(prediciton_BN,Brian_ONora_test$type)
...
plot(tree_model)
text(tree_model, use.n = T)
prediciton_BN<-predict(tree_model, newdata = Brian_ONora_AdjustedCalls, type = 'class')
confusionMatrix(table(prediciton_BN,Brian_ONora_AdjustedCalls$AdjustedCall))
